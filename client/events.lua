RegisterNetEvent('qb-repairstations:client:markAnimation', function()
    while true do
        Citizen.Wait(25)	
        if fixing then
...
function RepairVehicle(target, repairTime)
    ...
    TriggerEvent('qb-repairstations:client:markAnimation')	
...
RegisterNetEvent('qb-repairstations:client:fixCar', function(repairTime)
    local playerPed = GetPlayerPed(-1)
    local vehicle   = GetVehiclePedIsIn(playerPed, false)
    fixing = true
...
-- for Update checks
RegisterNetEvent("qb-repairstations:client:Update", function(state)
